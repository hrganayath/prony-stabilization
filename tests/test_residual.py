...
def test_residual_error_perfect_reconstruction():
    """Residual should be ~0 when reconstruction is exact."""
    t = np.arange(20)
    ...
    res, rel_res, rmse, rel_rmse = compute_residual_error(
...
def test_residual_error_fixed_window():
    """Fixed-window option should run and return nonnegative values."""
    t = np.arange(30)
    a_true = np.array([1.0, 0.5])
    omega_true = np.array([-0.1 + 0.5j, -0.2 - 0.3j])
